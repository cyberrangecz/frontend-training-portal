...
export class TrainingDefinitionEditComponent extends BaseComponent implements OnInit, OnChanges {
  ...
  @Output() trainingDefinitionSaved = new EventEmitter<TrainingDefinitionSaveEvent>();

  editMode: boolean;
  trainingConfigurationFormGroup: TrainingDefinitionEditFormGroup;
  ...
  get betaTestingGroup() {
    return this.trainingConfigurationFormGroup.formGroup.get('betaTestingGroup');
  }

  ngOnChanges(changes: SimpleChanges) {
    if (!this.trainingConfigurationFormGroup) {
      this.trainingConfigurationFormGroup = new TrainingDefinitionEditFormGroup();
    }
   ...
   */
  saveTrainingDef(continueToLevels = false) {
    if (this.trainingConfigurationFormGroup.formGroup.valid) {
      this.setInputValuesToTrainingDef();
      this.sendRequestToSaveChanges(continueToLevels);
    }
  }

  private onSaved(id: number, continueToLevels: boolean = false) {
    this.trainingDefinitionSaved.emit(new TrainingDefinitionSaveEvent(this.editMode, id, continueToLevels));
   ...
   */
  private sendRequestToSaveChanges(continueToLevels = false) {
    if (this.editMode) {
      this.updateTrainingDefinition(continueToLevels);
    } else {
    ...
    this.trainingDefinitionFacade
      ...
      .subscribe(
        id => {
          ...
          this.onSaved(id, continueToLevels);
        },
        err =>
